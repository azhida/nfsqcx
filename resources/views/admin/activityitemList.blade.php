...
<div class="x-body">
  <div class="layui-row">
    <form class="layui-form layui-col-md12 x-so" id="search_form">
      ...
      <input type="text" name="name"  placeholder="请输入活动品项名称" autocomplete="off" class="layui-input" value="{{ $search_params['name'] ?? '' }}">
  ...
  <xblock>
    ...
    <button class="layui-btn" onclick="x_admin_show('添加活动品项','{{ url('admin/activityitemAdd') }}')"><i class="layui-icon"></i>添加</button>
  ...
  <table class="layui-table">
    ...
    @foreach($list as $value)
      <tr>
        ...
        <td class="td-manage">
          <a title="编辑"  onclick="x_admin_show('编辑','{{ url('admin/activityitemEdit') . '/' . $value->id }}')" href="javascript:;">
...
<script>
    ...
    function member_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            //发异步删除数据
            $.ajax({
                url: '{{ url('admin/activityitemDelete') }}',
                type: 'post',
