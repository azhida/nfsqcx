...
<div class="x-body">
  <div class="layui-row">
    <form class="layui-form layui-col-md12 x-so" id="search_form">
      ...
      <input type="text" name="sales_name"  placeholder="请输入销售渠道名称" autocomplete="off" class="layui-input" value="{{ $search_params['sales_name'] ?? '' }}">
  ...
  <xblock>
    ...
    <button class="layui-btn" onclick="x_admin_show('添加销售渠道','{{ url('admin/saleschannelAdd') }}')"><i class="layui-icon"></i>添加</button>
  ...
  <table class="layui-table">
    ...
    @foreach($list as $value)
      <tr>
        ...
        <td class="td-manage">
          <a title="编辑"  onclick="x_admin_show('编辑','{{ url('admin/saleschannelEdit') . '/' . $value->id }}')" href="javascript:;">
...
<script>
    ...
    function member_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            //发异步删除数据
            $.ajax({
                url: '{{ url('admin/saleschannelDelete') }}',
                type: 'post',
